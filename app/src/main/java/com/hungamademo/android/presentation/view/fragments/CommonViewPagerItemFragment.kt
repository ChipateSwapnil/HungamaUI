package com.hungamademo.android.presentation.view.fragments
...
import com.hungamademo.android.presentation.adapter.BucketAdapter
import com.hungamademo.android.presentation.viewmodel.BucketItemViewModel
...
class CommonViewPagerItemFragment : Fragment() {
    ...
    private fun initView() {
        ...
        bottomNavigationSelected?.let {
            tabSelected?.let { it1 ->
                context?.let { it2 -> bucketViewModel?.getBucketData(it2, bottomNavigationSelected!!, tabSelected!!) }
                bucketViewModel?.bucket?.observe(this, Observer {
                    if (it != null && it.dataList.isNotEmpty()) {
                        buckets = it
                        bucketAdapter = BucketAdapter(buckets.dataList as ArrayList<BucketData>, requireContext())
